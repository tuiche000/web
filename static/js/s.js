...
document.onclick=function (ev){
  var s_base='http://www.mt.com:8081/s';
  ...
  while(src){
    if(src.dataset && src.dataset.bid){
      result.push({
        uid: (/uid=([a-z0-9]{32})/g.test(document.cookie),RegExp.$1),
        bid: src.dataset.bid,
        data: src.dataset.bdata
      })
    }
    src=src.parentNode;
  }
  ...
  //
  var url=s_base+'?q='+Base64.encode(JSON.stringify(result))+'&t='+Math.random();
  var img=new Image();  //<img />
  img.src=url;          //<img src="xxxx?xxx" />
};
